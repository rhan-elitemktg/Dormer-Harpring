...
// Query: *[_type == "featuredPracticeArea"] | order(label asc){  "key": key.current,  seo{ metaTitle, metaDescription },  hero{    "trail": coalesce(trail[]{ _key, label, href }, []),    title,    lede,    "proof": coalesce(proof[]{ _key, big, label, href, google }, []),    ctaLabel,    telLabel,    photoAlt,    reviewer{      "name": member->name,      "role": member->role,      "memberKey": member->key.current,      portrait,      updated    }  },  nav{    "items": coalesce(items[]{ _key, section, label }, []),    ctaLabel  },  triage{    title,    lede,    video{ poster, alt, title, length, "video": film{ provider, id } },    help{ text },    "rows": coalesce(rows[]{ _key, tone, tag, question, body, ctaLabel, ctaHref, stat{ big, label } }, []),    sources{ label, "items": coalesce(items[]{ _key, label, note, href }, []) }  },  takeaways{    eyebrow,    title,    lede,    "items": coalesce(items[]{ _key, title, body }, [])  },  criteria{    title,    lede,    video{ poster, alt, title, length, "video": film{ provider, id } },    "items": coalesce(items[]{ _key, title, body }, []),    note  },  lawyers{    title,    lede,    "attorneys": coalesce(attorneys[]{ _key, "key": member->key.current, cred }, []),    moreLabel,    moreHref  },  credentials{    "badges": coalesce(badges[]{ "_key": _key, "awardKey": @->key.current }, []),    eyebrow,    disclaimer  },  whyFirm{    eyebrow,    title,    "stats": coalesce(stats[]{ _key, big, label }, []),    disclaimer,    "columns": coalesce(columns[]{ _key, n, title, body }, []),    ctaLabel,    ctaHref,    photoAlt  },  results{    eyebrow,    title,    offeredLabel,    recoveredLabel,    "stories": coalesce(stories[]{      _key, offered, recovered, title, story, changed,      "reviewKey": review->key.current    }, []),    disclaimer  },  timeline{    title,    lede,    "steps": coalesce(steps[]{ _key, n, title, body }, []),    "phases": coalesce(phases[]{ _key, title, when, body }, []),    photoAlt,    "points": coalesce(points, [])  },  crashTypes{    title,    lede,    "tiles": coalesce(tiles[]{ _key, name, body, linkLabel, href }, [])  },  denver{    title,    lede,    "stats": coalesce(stats[]{ _key, big, label, body }, []),    "corridors": coalesce(corridors[]{ _key, name, body }, [])  },  checklistTeaser{    title,    body,    ctaLabel,    ctaHref,    "steps": coalesce(steps[]{ _key, iconKey, label }, [])  },  faultTeaser{    title,    body,    ctaLabel,    ctaHref,    scale{ start, middle, end },    source{ label, "items": coalesce(items[]{ _key, label, note, href }, []) }  },  more{    title,    "features": coalesce(features[]{ _key, title, body, length, poster, ctaLabel, href }, []),    "cards": coalesce(cards[]{ _key, title, body, ctaLabel, href }, [])  },  closing{ title, lede, officeLabel }}
