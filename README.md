...
| **"What to do after a car accident"** | The teaser now links `/what-to-do-after-a-car-accident-in-colorado/`, which covers the same ground — so no dead control. **The label is the open piece:** it says "See all 8 steps" and that article is not eight steps. The comp links a post (`DH - Blog - What to do after a car accident.html`) that carries the eight steps *and* the glove-box card, both cut from the Car Accidents page. Write it and repoint, or reword the button. Not a launch blocker. |
...
| **Cutover URLs** | **AUDITED AND CLOSED.** 369 live URLs resolved against this build: 323 served, 46 redirected, **0 that would 404**. Eighteen were found and every one is ruled on — see HANDOFF for the method, which matters more than the result: a sitemap and a REST index list only canonical DESTINATIONS, so a redirect SOURCE appears in neither and six of the eighteen were found only by deriving candidate URLs and probing them. **Re-run it before launch** — the live site is still publishing, which is how the 2026 gala write-up was missed by the sweep that caught the other 24. |
| **SEO layer** | **BUILT** — `/new-seo-setup` steps 3–11. Per-page `seo` on all 16 routed types, a Global SEO Settings singleton (crawl switch + default share image), `sitemap.xml`, `robots.txt`, and editor-managed redirects. **Every field is a fallback: 0 of 330 pages changed title or description.** Two things left: no default OG image is uploaded, and the two dynamic routes pass `title`/`description` rather than the whole `seo` object, so `canonicalUrl`/`noIndex`/`ogImage` are not yet settable per practice area, post or bio. |
| **Redirects for editors** | Two-tier, deliberately. The 196 cutover rules stay in `src/data/redirects.ts` → `vercel.json`; the SEO team adds new ones in Studio → Site Settings → Global SEO Settings → Redirects, delivered via `bulk-redirects.json`. A rule whose source is a live page is dropped at build time and logged — Vercel matches redirects before the filesystem, so it would otherwise take that page off the site. Hand the team `docs/redirects-for-editors.md`. **`bulkRedirectsPath` is currently OFF** — the first deploy failed on it (the deploy step runs from `/vercel/output`, so a repo-root path misses). Next value to try is `"static/bulk-redirects.json"`, on a preview deployment. Everything else in the redirect layer is built and tested. |
| **`sitemap.xml`** | ~~Still not built~~ **BUILT** as an endpoint (328 URLs, `noIndex` pages excluded). Was: not linked either. `/sitemap/` is the human page the footer points at; the XML file is a crawler artifact whose every URL is absolute off `site:` — **now settled on www**, so this is unblocked work rather than a pending decision. It belongs to `/new-seo-setup` with `robots.txt`. |
