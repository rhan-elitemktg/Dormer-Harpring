...
| **Blog posts** | The Blog comp's feed is thirteen cards, of which **five are real and eight are not**. The featured post and the first four grid cards are the live blog's June 2026 entries — same titles, same dates, same reviewer — and they carry their legacy slugs, so those links resolve. The other eight are copy the designer wrote: no post with those titles or dates exists among the 167 legacy posts, and four of them are the same placeholder articles the homepage's insights tab shows. They ship with `href: null` and render without a link. Replace them with real posts, or drop them, before launch. The legacy blog itself has 167 posts and 23 categories waiting on the Sanity migration. |
| **Blog post pages** | **One of the five real posts is built** — the trampoline-waiver article, which is the one the comp renders. The other four keep the legacy `href` they already had: live today, unserved by this build after cutover. They are not hand-transcribed on purpose, because the Sanity phase imports all 167 from the same scrape by script. Nothing else needs doing here first — the template is done. |
...
| **Car accident figures** | The detail page ships **seven unsourced numbers**, all the comp's. Four are city data — 22,400 crashes, 5,900 injury crashes, 84 fatal, 27% hit-and-run — attributed to "Colorado Department of Transportation and Denver Police Department, **[year]**", which is the comp's own placeholder. Three are the firm's own closed-case data — 3.4× over first offer, 41 cases to verdict, 1 in 5 calls told to skip a lawyer — under a visible "**[Methodology pending]**" line, again the comp's. Source the four or drop the band; write the methodology or drop the three. Colorado's advertising rules care about the second set. |
| **Statute citations** | The page cites eight Colorado statutes (10-4-635, 10-4-620, 10-4-609, 13-80-101, 13-21-111, 13-21-102.5, 24-10-109). The comp links **all of them to `law.justia.com/codes/colorado/`** — the index, not the section — so they ship as **text, not links**. Either point each at its own statute or leave them as citations. |
| **Glove box card** | "Download and print" has no artwork and no target; both its buttons currently scroll to the contact form. The QR square on the card's back is a drawn stand-in (`aria-hidden`) with no code and no landing page. Produce the PDF and the short link, or drop the card. |
...
| **Five supporting articles** | The detail page's Related column lists "Colorado filing deadlines", "Recorded statements", "MedPay explained", "Suing a government entity in Colorado" and "How comparative fault works". **None matches a post among the 167** — searched by topic as well as by title. They render as plain text. Commission them, retitle them onto the nearest real posts, or drop the column. |
