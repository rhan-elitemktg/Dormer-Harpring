...
| **`sitemap.xml`** | Still not built, and now not linked either. `/sitemap/` is the human page the footer points at; the XML file is a crawler artifact whose every URL is absolute off `site:` — **now settled on www**, so this is unblocked work rather than a pending decision. It belongs to `/new-seo-setup` with `robots.txt`. |
